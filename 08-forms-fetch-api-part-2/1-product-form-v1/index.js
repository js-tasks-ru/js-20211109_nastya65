...
export default class ProductForm {
  ...
  async render () {
    ...
    if (this.productId) {
      ...
      for (let key of Object.keys(this.formData)) {
        for (let [newKey, newValue] of Object.entries(newFormData)) {
          if (newKey === key) {
  ...
  imagesTemplate() {
    return this.formData.images.map(elem => this.imagesList(elem.url, elem.source)).join('');
  }
  ...
  uploadImage = () => {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      try {
        ...
        sortableList.innerHTML += this.imagesList(newImg.data.link, file.name);
      } catch (error) {
  ...
  getFormData() {
    ...
    const imagesInput = Array.from(imagesList).map((elem) => elem.getElementsByTagName('input'));
    data.images = imagesInput.map(elem => {
      const imageObj = {};
      imageObj['url'] = Array.from(elem)[0].value;
      imageObj['source'] = Array.from(elem)[1].value;
      return imageObj;
    })
